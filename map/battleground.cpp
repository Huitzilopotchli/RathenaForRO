...
int bg_block_skill_end(int tid, unsigned int tick, int id, intptr_t data)
{
	...
	char output[128];
	int idx = battle_config.guild_skill_relog_delay ? (int)data - GD_SKILLBASE : 0;
...
void bg_block_skill_status(struct battleground_data *bg, int skillnum)
{
	...
	idx = battle_config.guild_skill_relog_delay ? skillnum - GD_SKILLBASE : 0;
...
{
	int idx = battle_config.guild_skill_relog_delay ? skillnum - GD_SKILLBASE : 0;
...
{ // Player joins team
	...
	struct battleground_data *bg = bg_team_search(bg_id);
	struct map_session_data *pl_sd;
	struct block_list *bl = map_id2bl(BL_PC);
...
//	int i;
	...
	for( i = 0; i < MAX_BG_MEMBERS; i++ )
	{
		...
		if (battle_config.bg_idle_autokick && DIFF_TICK(last_tick, sd->idletime) >= battle_config.bg_idle_autokick && bg->g)
		{
			...
			pc_setpos(sd, sd->status.save_point.map, sd->status.save_point.x, sd->status.save_point.y, CLR_RESPAWN);
			continue;
		}
...
void bg_guild_build_data(void)
{
	...
	for (i = 1; i <= MAX_BATTLEGROUND_TEAMS; i++)
	{ // Emblem Data - Guild ID's
		...
		if ((fp = fopen(path, "rb")) != NULL)
		{
			...
			fclose(fp);
			ShowStatus("Done reading '" CL_WHITE "%d" CL_RESET "' emblem data file.\n", path);
		}
	}
...
void bg_team_rewards(int bg_id, int nameid, int amount, int kafrapoints, int quest_id, const char *var, int add_value, int bg_arena, int bg_result)
{
	...
	for (j = 0; j < MAX_BG_MEMBERS; j++)
	{
		...
		pc_setglobalreg(sd, add_str(var), + add_value);
...
int queue_join(struct map_session_data *sd, int q_id)
{
	...
	if (battle_config.bg_from_town_only && !map[sd->bl.m].flag.town)
	{
	...
	if (battle_config.bg_ip_check && battleground_countlogin(sd, false) > 0)
	{
...
static int bg_team_db_reset(DBKey key, struct DBData *data, va_list ap)
{
...
static int queue_db_final(DBKey key, struct DBData *data, va_list ap)
{
